...
# Base directory for locally stored photos — /app/uploads in Docker
...
def _photo_local_url(photo: Photo) -> str | None:
    ...
    if photo.local_path:
        return f"/photos/{photo.local_path}"
    return None
...
async def _download_and_store_photo(
    ...
    # Store just the filename (served via StaticFiles at /photos/)
    photo.local_path = safe_name
    logger.info(f"Saved photo {photo.id} to {file_path}")


async def _auto_match_track(
    db: AsyncSession,
    user: User,
    memory: Memory,
    photo: Photo,
    ...
    """Run the resolve → join → rank pipeline and create a TrackPhotoMapping if a match is found."""
    ...
    memory_date_only = memory_date.date() if hasattr(memory_date, 'date') else memory_date
    ...
    if not candidate_tracks:
        ...
        if track_tuples:
            ...
            if matched_ids and desc and desc.strip():
                try:
                    mood_result = _mood_matcher.match(
                        desc, matched_ids, top_n=len(matched_ids)
                    )
                    if mood_result and mood_result.ranked:
                        logger.info(
                            f"\n{'='*60}\n"
                            f"🎯 MOOD MATCHING for memory '{title}'\n"
                            f"   Description: \"{desc}\"\n"
...
async def _refetch_memory(db: AsyncSession, memory_id: int) -> MemoryResponse:
    """Re-fetch a memory with eager loading and return the response."""
...
async def create_memory(
    ...
    """Create a memory with one optional photo (JSON body, supports data: URIs and Google Photos)."""
    db_memory = Memory(
        user_id=user.id,
        title=memory.title,
    ...
    await db.commit()
    return await _refetch_memory(db, db_memory.id)
...
async def upload_memory(
    ...
    """Create a memory with a multipart file upload."""
    from datetime import datetime as dt

    parsed_date = dt.fromisoformat(memory_date)

    os.makedirs(UPLOAD_DIR, exist_ok=True)

    # Generate a unique filename
    original_ext = os.path.splitext(photo.filename or "photo.jpg")[1] or ".jpg"
    ...
    db_photo = Photo(
        memory_id=db_memory.id,
        google_photo_id=f"local_{uuid.uuid4().hex[:12]}",
        base_url="",
        filename=photo.filename or "photo.jpg",
        mime_type=photo.content_type or "image/jpeg",
        creation_time=parsed_date,
